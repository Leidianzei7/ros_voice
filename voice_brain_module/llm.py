...
def build_system_prompt() -> str:
    ...
    return (
        ...
        "- 视觉识别：可识别桌面物体，可抓取物体为苹果、香蕉、瓶子、蛋糕；"
        "上下文以「物体×数量」标注（如「苹果×2」）\n"
        "- 情绪识别：可识别用户情绪状态\n\n"
        "⚠️ 若消息中无【当前桌面物体】/【用户情绪】标记，说明没有感知数据。"
        "被问到\"看到了什么\"时必须诚实回答，**严禁编造物体名称**。\n\n"
        "【回复格式】严格按以下格式，两个标签都必须出现：\n"
        "[口语回复]：\n"
        "<自然口语回答；若有动作请求，在末尾用一句话确认动作>\n"
        "[执行指令]：\n"
        ...
        "【当前桌面物体】共3个：苹果×2, 杯子\n【可抓取物体】苹果×2\n\n---\n用户说：桌子上有什么\n"
        "[口语回复]：\n"
        "我看到桌上有两个苹果和一个杯子，需要我帮您拿什么吗？\n"
        "[执行指令]：\n"
        "[]\n\n"
        "示例 6（无视觉数据）：\n"
        "用户：桌子上有什么\n"
        "[口语回复]：\n"
        "我现在还看不到桌面，视觉功能还没有数据。您需要什么，我来帮您想办法。\n"
        "[执行指令]：\n"
        "[]\n\n"
        "示例 7（不在指令集）：\n"
        "用户：打开窗帘\n"
        ...
        "- 只有动作没有问询：[口语回复] 简短确认（如\"好的，马上去\"）\n"
        "- 无视觉数据时，回答物体相关问题必须诚实，不编造物体名称\n"
        "- 用户谈及桌面物体但未要求抓取时，描述所见即可，不自动生成抓取指令\n"
        "- 参数值必须严格匹配选项，不做近义替换\n"
        "- 不输出两个标签之外的任何内容\n"
...
def _call_llm(messages: list) -> str:
    ...
    stream = llm_client.chat.completions.create(
        ...
        extra_body={
            "enable_search": True,
            "forced_search": True,   # 强制搜索：turbo 自行判断偏保守，常漏搜
        },
    )
    raw = ""
    for chunk in stream:
...
def classify_meta_response(user_text: str) -> str:
    ...
    messages = [
        {"role": "system", "content": (
            ...
            "NOT_ADDRESSING — 用户的话不是在回应机器人，而是在跟房间里的其他人说话，"
            "或者是自言自语、背景噪音。特征是：出现了其他人的名字或称呼、"
            "对话内容与机器人的问句毫无关联。\n\n"
            "REJECTING — 用户在回应机器人，但明确表示不是在和机器人说话、"
            "或者驱赶机器人。例子：\"没有\"、\"不是\"、\"没跟你说话\"、"
            "\"你一边去\"、\"别烦我\"、\"走开\"。"
            "注意：即使用户说的是\"没有\"这种简短否定，只要它是在回答机器人的元问题，"
            "就属于 REJECTING 而非 NOT_ADDRESSING。\n\n"
            "ADDRESSING — 用户在回应机器人，且没有拒绝的意思。"
            "例子：\"对，我在跟你说话\"、\"是我在叫你\"、\"帮我拿个东西\"。\n\n"
