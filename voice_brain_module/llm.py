...
def build_system_prompt() -> str:
    ...
    return (
        ...
        "示例 3（可执行动作，确定要做什么）：\n"
        "用户：帮我抓一下<某物>\n"
        "（<某物>在可用指令集的抓取选项中，确定可以执行）\n"
        "[口语回复]：\n"
        "好的，我这就帮您拿过来。\n"
        "[执行指令]：\n"
        "[{\"actuator\":\"机械臂\",\"action\":\"抓取\",\"params\":{\"target\":\"<从options中选取>\"}}]\n\n"
        "示例 4（多个指令组合）：\n"
        "用户：前进两米然后抓<某物>\n"
        "[口语回复]：\n"
        "好的，我先前进再帮您拿。\n"
        "[执行指令]：\n"
        "[{\"actuator\":\"底盘\",\"action\":\"前进\",\"params\":{\"speed\":0.2,\"distance\":2.0}},{\"actuator\":\"机械臂\",\"action\":\"抓取\",\"params\":{\"target\":\"<从options中选取>\"}}]\n\n"
        "示例 5（有视觉数据）：\n"
        "【当前视野物体】共N个：<物体>×<数量>, ...\n【可抓取物体】<物体>×<数量>\n\n---\n用户说：看到什么了\n"
        "[口语回复]：\n"
        ...
        "好的，给您放一首歌。\n"
        "[执行指令]：\n"
        "[{\"actuator\":\"音箱\",\"action\":\"播放歌曲\",\"params\":{\"song\":\"<从可用指令集的options中选取>\"}}]\n\n"
        "示例 8（信息不足，只问不做）：\n"
        "用户：放首歌吧\n"
        "（用户没说是哪首，曲库有多首可选）\n"
        "[口语回复]：\n"
        "曲库里有<实际歌名1>和<实际歌名2>，您想听哪一首呢？\n"
        "[执行指令]：\n"
        "[]\n\n"
        "示例 9（不在指令集）：\n"
        "用户：打开窗帘\n"
        ...
        "  错误示例（禁止）：\n"
        "    用户：放首歌 → LLM：\"想听XX吗？\" + 放XX  ← 大错（问了又做）\n"
        "    用户：拿个水果 → LLM：\"要XX还是YY？\" + 抓XX  ← 大错（问了又做）\n"
        "  正确示例：\n"
        "    用户：放首歌 → LLM：\"曲库里有XX和YY，您想听哪首？\" + []\n"
        "    （注意：XX/YY必须是可用指令集中实际列出的歌名，不许编造）\n"
        "    用户：拿个水果 → LLM：\"有XX和YY，您要哪个？\" + []\n"
        "  ★ 宁可多问一句，绝不猜错执行。"
