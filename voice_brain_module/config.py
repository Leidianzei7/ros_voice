...
# ── 紧急呼叫发起（负面情绪触发）─────────────────────────────
# 检测到稳定的负面情绪时，机器人先问一句，等 ASK_WAIT_SEC 秒；期间用户明确
# 拒绝就作罢，没应答（可能已无法回应，正是该求助的情况）则发起紧急联络。
USER_NAME = "肖奶奶"           # 称呼，用于固定话术

# 情绪 → 联络方式。call = 拨打紧急联系人电话；sms = 发送短信。
# 当前两档都只发短信——短信不打扰家属接听，可回溯，也不会因为一次误判就
# 把人从工作里叫出来。`call` 仍然是有效取值，将来要给某档升级为电话，改这里即可。
# **未列出的情绪不触发联络流程**，仍走原来的大模型安抚——将来情绪侧新增情绪类型时，
# 默认不会突然开始联络家属，必须有人显式加到这张表里。
